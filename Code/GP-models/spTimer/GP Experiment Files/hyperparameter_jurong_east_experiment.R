...
#Original Data used:
dir <- "C:/Users/samue/Documents/Masters/Sem 2/ST5188/st5188/Data/Final/final_JURONG EAST_long.csv"
data <- read.csv(dir)
#Define Bounding Box to reduce computational complexity 
resolution <- get_resolution(data)
x_min <- 103.7158 + 20*resolution   
x_max <- 103.7158 + 30*resolution  
y_min <- 1.301308 + 25*resolution 
y_max <- 1.301308 + 35*resolution   
...
priors <- list(
  spT.decay(distribution = Gamm(1, 1), tuning = 2),
  spT.decay(distribution = Gamm(1/4, 1/4), tuning = 2),
  spT.decay(distribution = Gamm(4, 2), tuning = 2),
  spT.decay(distribution = Gamm(1, 1/2), tuning = 2),
  spT.decay(distribution = Gamm(5, 1), tuning = 2),
  spT.decay(distribution = Gamm(6.25, 1.25), tuning = 2),
  spT.decay(distribution = Gamm(0.01, 0.01), tuning = 2)
)



run_hyperparameter_experiment <- function(priors,cov_fn,n_samples){
  output_df <- data.frame(ID = c("1","3","6","9","12","overall_RMSE"))
  get_RMSE_by_prior <- function(prior,cov_fn = "spherical",n_samples = 10){
    set.seed(5188)
  ...
  for (prior in priors){
    average_LST <- get_RMSE_by_prior(prior=prior,cov_fn,n_samples = n_samples)
...
output_df <- run_hyperparameter_experiment(priors,"spherical",n_samples = 10) 
colnames(output_df) <- c("ID", 1:length(priors))
write.csv(output_df, "hyperparameter_jurong_east_experiment.csv")
