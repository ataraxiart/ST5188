...
#Original Data used:
dir <- "/Users/samue/Documents/Masters/Sem 2/ST5188/st5188/Data/Final/final_JURONG WEST_long.csv"
data <- read.csv(dir)
#Define Bounding Box to reduce computational complexity 
resolution <- get_resolution(data)

x_min <- 103.6883 + 20*resolution   
x_max <- 103.6883 + 30*resolution  
y_min <- 1.340466 - 10 *resolution 
y_max <- 1.340466 + 0*resolution   
...
priors <- list(
  spT.decay(distribution = Gamm(1, 1), tuning = 6),
  spT.decay(distribution = Gamm(1/4, 1/4), tuning = 6),
  spT.decay(distribution = Gamm(4, 2), tuning = 6),
  spT.decay(distribution = Gamm(1, 1/2), tuning = 6),
  spT.decay(distribution = Gamm(5, 1), tuning = 6),
  spT.decay(distribution = Gamm(6.25, 1.25), tuning = 6),
  spT.decay(distribution = Gamm(0.01, 0.01), tuning = 6)
)
...
write.csv(output_df, "hyperparameter_jurong_west_experiment.csv")
