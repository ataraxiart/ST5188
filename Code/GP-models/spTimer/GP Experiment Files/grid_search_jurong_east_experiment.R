...
#Original Data used:
dir <- "/Users/samue/Documents/Masters/Sem 2/ST5188/st5188/Data/Final/final_JURONG EAST_long.csv"
data <- read.csv(dir)
...
priors <- list(
  spT.decay(distribution = Gamm(1, 1), tuning = 3),
  spT.decay(distribution = Gamm(1/4, 1/4), tuning = 3),
  spT.decay(distribution = Gamm(4, 2), tuning = 3),
  spT.decay(distribution = Gamm(1, 1/2), tuning = 3),
  spT.decay(distribution = Gamm(5, 1), tuning = 3),
  spT.decay(distribution = Gamm(6.25, 1.25), tuning = 3),
  spT.decay(distribution = Gamm(0.01, 0.01), tuning = 3)
)



#
phi_lst <- seq(0.1, 2, by = 0.1)

run_grid_search_experiment <- function(phi_lst,n_samples,cov_fn){
  output_df <- data.frame(ID = c("1","3","6","9","12","overall_RMSE"))
  get_RMSE_by_phi <- function(phi,cov_fn,n_samples = 10){
    set.seed(5188)
  ...
  for (phi in phi_lst){
    average_LST <- get_RMSE_by_phi(phi=phi,cov_fn = cov_fn,n_samples = n_samples)
...
output_df <- run_grid_search_experiment(phi_lst,"spherical",n_samples = 10) 
colnames(output_df) <- c("ID", phi_lst)
write.csv(output_df, "grid_search_jurong_east_experiment.csv")
