...
run_RMSE_experiment <- function(lag_lst,n_samples){
  output_df <- data.frame(ID = c("1","3","6","9","12","overall_RMSE"))
  get_RMSE_by_lags <- function(num_lags,n_samples = 10){
    set.seed(5188)
    ...
    new_formula <- get_formula('avg_LST',num_lags)
    iteration_list <- list()
    ...
    for (i in 1:n_samples){
      ...
      sum_{i=1}^{N} (sum_{j=1}^{12} (f_ij -\hat{f_ij})^2 )
  ...
  for (lag in lag_lst){
    average_LST <- get_RMSE_by_lags(lag,n_samples = n_samples)
...
output_df <- run_RMSE_experiment(c(1,2,3,4,5,6),n_samples = 10) 
colnames(output_df) <- c(c("ID"),1:6)
